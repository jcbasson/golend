...
const VehiclePicture = ({ year, name }) => {
  ...
  const lowQualityVehiclePicSrc = `${cdnUrl}/w_170/e_blur:1000,q_1,f_auto/e_grayscale/${cdnVersion[year]}/${imageFolderPath}/${year}/${name}.${imageFormat}`;
  const highQualityVehiclePicSrc = `${cdnUrl}/w_170,h_150/${cdnVersion[year]}/${imageFolderPath}/${year}/${name}.${imageFormat}`;
